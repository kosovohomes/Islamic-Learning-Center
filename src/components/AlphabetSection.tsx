...
export default function AlphabetSection() {
  ...
  const handleSpeak = (text: string) => {
    ...
    utterance.onerror = (e) => {
      ...
      setIsPlaying(false);
      setFeedback("Vocal synthesis was blocked or not supported. Phonetic: " + selectedLetter.phonetic);
        ...
        <div className="grid grid-cols-4 sm:grid-cols-7 gap-2.5">
          {ARABIC_LETTERS.map((item, index) => {
            ...
            return (
              <button
                key={item.char}
                onClick={() => { setSelectedLetter(item); setFeedback(""); }}
                className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all relative group cursor-pointer ${
                  isSelected ? "bg-emerald-500 border-emerald-500 text-white shadow-md shadow-emerald-500/10 scale-102" : "bg-slate-50 border-slate-100 text-slate-800 hover:bg-slate-100/70 hover:border-slate-200"
                }`}
              >
                <span className="text-2xl font-semibold mb-0.5 leading-none font-sans">{item.char}</span>
                <span className={`text-[10px] uppercase font-mono tracking-wider ${isSelected ? "text-emerald-100" : "text-slate-400"}`}>{item.name}</span>
                <span className="absolute top-1 right-1 text-[8px] font-mono text-slate-300 group-hover:block hidden">{index + 1}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="bg-gradient-to-b from-emerald-950 to-slate-900 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden flex flex-col flex-1 min-h-[400px]">
          ...
          <div className="flex items-center gap-6 mb-6 z-10">
            <div className="w-24 h-24 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center relative">
              <span className="text-5xl font-sans font-bold text-emerald-100 select-none">{selectedLetter.char}</span>
            </div>
            <div className="flex-1">
              <h3 className="text-2xl font-sans font-semibold tracking-tight text-white flex items-center gap-2">{selectedLetter.name}</h3>
              <p className="text-xs font-mono text-slate-400 mt-0.5">Phonetic: <span className="text-emerald-300 underline font-semibold">/{selectedLetter.phonetic}/</span></p>
              <button onClick={() => handleSpeak(selectedLetter.exampleWord)} disabled={isPlaying} className="mt-3 flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-medium px-4 py-2 rounded-xl text-xs transition-all tracking-tight cursor-pointer shadow-lg shadow-emerald-500/10 disabled:opacity-50">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-6 z-10">
            ...
            <div className="grid grid-cols-4 gap-2 text-center">
              {[{label: "Isolated", val: selectedLetter.isolated}, {label: "Initial", val: selectedLetter.initial}, {label: "Medial", val: selectedLetter.medial}, {label: "Final", val: selectedLetter.final}].map(({label, val}) => (
                <div key={label} className="p-2 bg-slate-800/50 rounded border border-white/5">
                  <div className="text-lg font-semibold h-7 flex items-center justify-center text-white">{val}</div>
