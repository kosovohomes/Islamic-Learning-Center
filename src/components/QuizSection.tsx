      ...
      <div className="lg:col-span-4 flex flex-col gap-4">
                ...
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl border text-xs font-semibold tracking-tight transition-all flex items-center justify-between cursor-pointer ${
                    isSelected
                      ...
                      : "bg-slate-50 border-slate-100 text-slate-600 hover:bg-slate-100/40"
                  }`}
                >
        ...
        <div className="bg-emerald-950 text-white rounded-2xl p-5 border border-emerald-900 shadow-xl flex flex-col justify-between">
          ...
          <div className="grid grid-cols-2 gap-4 my-4 border-y border-white/5 py-4">
            ...
            <div className="text-center">
              <span className="text-3xl font-bold font-mono text-slate-200 block">
          ...
          <p className="text-[10px] text-emerald-300/80 italic text-center">
        ...
        {!showSummary && activeQuestion ? (
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <span className="text-[9px] font-mono uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-emerald-100 text-emerald-800">
            ...
            {/* Prompt String */}
            <div>
              <h3 className="text-base sm:text-lg font-sans font-medium text-slate-950 leading-relaxed">
            ...
            <div className="space-y-2.5">
              {activeQuestion.options.map((option, idx) => {
                ...
                const isCorrectStr = idx === activeQuestion.correctAnswerIndex;
                
                let buttonStyle = "bg-slate-50 border-slate-150 text-slate-800 hover:bg-slate-100/60";
                
                if (hasSubmitted) {
                  if (isCorrectStr) {
                    buttonStyle = "bg-emerald-50 border-emerald-500 text-emerald-900 font-semibold shadow-xs";
                  } else if (isSelected && !isCorrectStr) {
                    buttonStyle = "bg-rose-50 border-rose-400 text-rose-800 shadow-xs";
                  } else {
                    buttonStyle = "bg-slate-50 border-slate-150/40 text-slate-400 pointer-events-none opacity-40";
                  }
                } else if (isSelected) {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3.5 rounded-xl border transition-all text-xs flex items-center justify-between cursor-pointer ${buttonStyle}`}
                  >
                    <span>{option}</span>
                    <div className="flex items-center gap-2">
                      {hasSubmitted && isCorrectStr && <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />}
                      {hasSubmitted && isSelected && !isCorrectStr && <XCircle className="w-4 h-4 text-rose-600 shrink-0" />}
                      <span className="text-[9px] font-mono text-slate-400 uppercase tracking-widest">{idx + 1}</span>
                    </div>
                  </button>
                );
              })}
            ...
            {/* Explanations and Submit triggers */}
            <div className="mt-4">
              {hasSubmitted ? (
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 mt-4">
                  ...
                  <button
                    ...
                    className="mt-4 w-full sm:w-auto flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold py-2.5 px-6 rounded-xl text-xs tracking-tight transition-all cursor-pointer shadow-md shadow-emerald-500/10 ml-auto"
                  >
                    Next Question
                ...
                <button
                  ...
                  className="w-full sm:w-auto flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-850 text-white font-semibold py-2.5 px-6 rounded-xl text-xs tracking-tight transition-all cursor-pointer ml-auto disabled:opacity-40 disabled:pointer-events-none"
                >
                  Submit Answer
                </button>
              )}
          ...
          /* High-Fidelity Badge summary card */
          <div className="text-center py-6 flex flex-col items-center justify-center max-w-lg mx-auto space-y-4">
            <span className="text-6xl animate-bounce duration-1000">
              {getBadgeNameAndTitle(score, filteredQuestions.length).emoji}
            </span>
            ...
            <div className="space-y-1">
              ...
              <h4 className="text-lg font-bold text-emerald-800 tracking-tight">
                {getBadgeNameAndTitle(score, filteredQuestions.length).name}
              </h4>
            </div>
            ...
            <button
              ...
              className="flex items-center gap-2 bg-slate-900 hover:bg-slate-850 text-white font-semibold py-2.5 px-6 rounded-xl text-xs tracking-tight transition-all cursor-pointer shadow-md shadow-slate-950/10"
            >
          ...
          <div className="text-center py-10 text-slate-400">
            No questions found in this difficulty pool. Please choose another pool in the sidebar.
          </div>
        )}
      </div>
    </div>
